...
def add(P, Q):
    ...
    if P != Q: lam = ((y2 - y1) * inv((x2 - x1) % p, p)) % p
    else:      lam = ((3*x1*x1 + A) * inv((2*y1) % p, p)) % p
    x3 = (lam*lam - x1 - x2) % p
    y3 = (lam*(x1 - x3) - y1) % p
    return (x3, y3)
...
def kdf_from_d(d: int) -> bytes:
    return hashlib.sha256(str(d).encode()).digest()  # 32-byte key
...
# --- 2) 本地校验：解密应还原为原 flag ---
pt = xor_keystream(bytes.fromhex(ct), K).decode()
assert pt == flag
